...
export function CampaignBuilderChat({ onResponse, lang = "ru" }: Props) {
  ...
  const [activeBuilderResponse, setActiveBuilderResponse] = useState<BuilderResponse | null>(activeDialog?.lastResponse ?? null);
  ...
  useEffect(() => {
    ...
    setActiveBuilderResponse(activeDialog.lastResponse ?? null);
  ...
  useEffect(() => {
    ...
    setDialogs((current) => current.map((dialog) => {
      ...
      return {
        ...dialog,
        messages,
        lastResponse: activeBuilderResponse,
        preferences,
        title: titleFromMessages(messages, dialog.title),
        updatedAt: new Date().toISOString(),
      };
    }));
    onResponse(activeBuilderResponse);
  }, [messages, activeBuilderResponse, preferences, activeDialog?.id, onResponse]);
  ...
  const handleSend = async () => {
    ...
    if (data) {
      setActiveBuilderResponse(data as BuilderResponse);
    }
  };
  ...
  return (
    <div className="fw-builder-chat">
      ...
      {/* Status bar */}
      {(activeBuilderResponse?.campaign_id || messages.length > 0 || hasPreferences(preferences)) && (
        <div className="fw-statusbar">
          {activeBuilderResponse?.campaign_id ? (
            <>
              <span>
                Campaign{" "}
              ...
              <span style={{ color: STATUS_COLORS[activeBuilderResponse.status] ?? "inherit", fontWeight: 600, fontSize: 12 }}>
                {STATUS_LABELS[lang][activeBuilderResponse.status] ?? activeBuilderResponse.status}
              </span>
            </>
          ) : (
