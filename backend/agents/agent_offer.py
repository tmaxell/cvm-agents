"""OfferAgent — генерация 2-3 вариантов контента коммуникации для кампании.
...
В терминах платформы «оффер» — это шаблон offerTemplate/businessOperation
на BusinessTransactionActivity; то, что генерирует этот агент — собственно
текст коммуникации (SMS/Push/Email body). Внутренние имена артефактов и
action id (offer_choice / offer_variants / select_offer / generate_offers)
...
NAME = "offer"
DESCRIPTION = "Генерирует 2-3 варианта контента коммуникации под продукт/канал/аудиторию с разными hook'ами."
...
Сгенерируй РОВНО 3 варианта контента коммуникации для одной кампании. Они
должны отличаться ключевым «крючком» (hook), а не быть переформулировками
одного текста.
...
Если в запросе scenario == "upsell_with_reminder" — это апсейл тарифа через SMS,
который собирается по шаблону с откликом «Ок» на короткий номер. В этом случае:
- Начни с обращения «Уважаемый абонент,».
- Используй CVM-плейсхолдеры дат вида {{[c.BeginDate]}} и {{[c.EndDate]}}
  (двойные фигурные скобки сохраняй ровно так — это синтаксис AdTarget).
- В тексте описывай период действия предложения и СТА: «отправьте SMS «Ок»
  на 999». Пример: «Уважаемый абонент, с {{[c.BeginDate]}} по {{[c.EndDate]}}
  вы можете бесплатно перейти на новый ТП Семейный — отправьте «Ок» на 999.»
- Hook определяет, чем варианты отличаются (скидка / срочность / выгода
  на каждый подключаемый номер и т. п.).

Верни строго JSON одной строкой:
{"variants":[
  {"id":"v1","hook":"<короткое описание крючка>","text":"<полный текст коммуникации>",
...
async def execute(ctx: AgentContext) -> AgentResult:
    ...
    scenario = inputs.get("scenario") or "default"
    char_limit = _channel_limit(channel)

    await ctx.emit(
        "step_started",
        detail=f"OfferAgent: product={product}, channel={channel}, scenario={scenario}",
    )
    ...
    if not variants:
        return AgentResult(
            assistant_message="Не удалось сгенерировать варианты контента коммуникации. Попробуйте уточнить продукт и аудиторию.",
            status="error",
        )
    ...
    # Рендерим карточки с кнопками «Выбрать этот вариант контента».
    lines = [
        f"Подобрал {len(variants)} вариант(а/ов) контента коммуникации для **{product}** "
...
def _fallback_variants(*, product: str, channel: str, audience: str, char_limit: int) -> list[dict[str, Any]]:
    """Резервный набор контента коммуникации, если LLM недоступен."""
    base = [
        (
            "Скидка",
